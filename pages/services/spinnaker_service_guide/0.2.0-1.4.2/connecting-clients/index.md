...
In general, the `.autoip.dcos.thisdcos.directory` endpoints will only work from within the same DC/OS cluster. From outside the cluster, you can either use direct IPs or set up a proxy service that acts as a front end to your Spinnaker instance. For development and testing purposes, you can use [DC/OS Tunnel](https://docs.mesosphere.com/1.10/administering-clusters/sshcluster/) to access services from outside the cluster, but this option is not suitable for production use.
...
This JSON array contains a list of valid nodes that you can use to connect to the spinnaker cluster. To ensure availability, it is best to specify multiple nodes in your configuration. Use the VIP to address any one of the Spinnaker nodes in the cluster.
...
## Accessing the Spinnaker UI with Edge-LB configuration
...
For Edge-LB pool configuration:
  ...
  ```  
  3. Create the configuration JSON file with required parameters to access Spinnaker web:
