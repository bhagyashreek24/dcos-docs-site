...
title: Managing
menuWeight: 80
excerpt: Managing your DC/OS Spinnaker configuration
featureMaturity:
enterprise: false
---
...
You can make changes to the service after it has been launched. Configuration management is handled by the scheduler process, which in turn handles Spinnaker deployment itself.

After making a change, the scheduler will be restarted, and it will automatically deploy any detected changes to the service, one node at a time. For example, a given change will first be applied to `spinnaker-0`, then `spinnaker-1`, and so on.
...
+ The service's subcommand available and installed on your local machine.
  + You can install just the subcommand CLI by running `dcos package install --cli --yes spinnaker`.
  + If you are running an older version of the subcommand CLI that doesn't have the `update` command, uninstall and reinstall your CLI.
    ```shell
    dcos package uninstall --cli spinnaker
    dcos package install --cli spinnaker
    ```
...
```shell
dcos spinnaker describe > options.json
```

Make any configuration changes to the `options.json` file.

If you installed this service with a prior version of DC/OS, this configuration will not have been persisted by the the DC/OS package manager. You can instead use the `options.json` file that was used when [installing the service](https://github.com/rathohit311356/dcos-docs-site/blob/Spinnaker-Service-Guide/pages/services/spinnaker/0.1.0-2.2.1/install/install.md).
...
First, we'll fetch the default application's environment, current application's environment, and the actual spinnaker that maps config values to the environment:
...
5. To identify those values that are custom, we'll get the default environment variables for this version of the service:
	...
	```
6. We'll also get the entire application spinnaker:
	```shell
...
1. Use JQ and `diff` to compare the two:
	...
	```
2. Now compare these values to the values contained in the `env` section in application spinnaker:
	```shell
...
To see a full listing of available options, run `dcos package describe --config spinnaker` in the CLI, or browse the DC/OS spinnaker Service install dialog in the DC/OS Dashboard.
...
```shell
10.0.10.3: spinnaker-0
10.0.10.8: spinnaker-1
10.0.10.26: spinnaker-2
10.0.10.28: empty
10.0.10.84: empty
```
...
```
    2. Redeploy `_NODEPOD_-1` from the decommissioned node to somewhere within the new whitelist: `dcos spinnaker pod replace _NODEPOD_-1`
...
1. Run `dcos spinnaker pod replace spinnaker-<NUM>`, e.g. `spinnaker-2` to halt the current instance with id `<NUM>` (if still running) and launch a new instance on a different agent.

For example, let's say `spinnaker-2`'s host system has died and `spinnaker-2` needs to be moved.

1. NOW THAT THE NODE HAS BEEN DECOMMISSIONED, (IF NEEDED BY YOUR SERVICE) start `spinnaker-2` at a new location in the cluster.
    ```shell
    dcos spinnaker pod replace spinnaker-2
    ```
...
```shell
dcos spinnaker update force-complete service-phase service-0:[node]
```
...
```shell
dcos spinnaker update force-restart
```

Or for all steps in a single phase:
```shell
dcos spinnaker update force-restart service-phase
```

Or for a specific step within a specific phase:
```shell
dcos spinnaker update force-restart service-phase service-0:[node]
```
