...
title: Uninstalling
menuWeight: 40
excerpt: Uninstalling DC/OS Apache Spinnaker Service
featureMaturity:
enterprise: false
---
...
For example, to uninstall the Apache Spinnaker instance named `spinnaker-dev`, run:

```shell
dcos package uninstall --app-id=spinnaker-dev spinnaker
```
...
```shell
dcos spinnaker --name=spinnaker plan show deploy
deploy (serial strategy) (IN_PROGRESS)
└─ node (serial strategy) (IN_PROGRESS)
   ├─ spinnaker-0:[init] (COMPLETE)
   ├─ spinnaker-0:[node] (COMPLETE)
   ├─ spinnaker-0:[metrics] (STARTING)
   ├─ spinnaker-1:[node] (PENDING)
   └─ spinnaker-1:[metrics] (PENDING)   
```    
...
└─ node (serial strategy) (COMPLETE)
   ├─ spinnaker-0:[init] (COMPLETE)
   ├─ spinnaker-0:[node] (COMPLETE)
   ├─ spinnaker-0:[metrics] (COMPLETE)
   ├─ spinnaker-1:[node] (COMPLETE)
   └─ spinnaker-1:[metrics] (COMPLETE)
```       
...
1. CLI: dcos spinnaker --name=spinnaker plan show deploy
2. HTTP: https://yourcluster.com/service/spinnaker/v1/plans/deploy/forceComplete?phase=unreserve-resources&step=unreserve-<UUID>
...
│  ├─ kill-task- (COMPLETE)
│  ├─ kill-task-spinnaker-0-init__4ea667b6-e784-4982-9c68-61af8732f552 (COMPLETE)
│  ├─ kill-task-spinnaker-0-metrics__d438f4b5-d6d1-480c-8fd7-439943f5cba9 (COMPLETE)
│  ├─ kill-task-spinnaker-0-node__3e16d999-0b72-4ca2-bee4-74c910acd8a3 (COMPLETE)
│  ├─ kill-task- (COMPLETE)
│  ├─ kill-task- (COMPLETE)
│  ├─ kill-task-spinnaker-1-metrics__d0d1b9ca-e803-4392-9210-8088ee234df4 (COMPLETE)
│  ├─ kill-task-spinnaker-1-node__a7704212-ce1f-4150-aa75-5f08a5fabbab (COMPLETE)
│  └─ kill-task- (COMPLETE)
...
```shell
       dcos package uninstall --app-id=/test/spinnaker spinnaker
```                   
